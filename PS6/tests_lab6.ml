...
[ 
  ...
  "Problem B.3" >:: (fun _ ->
     ...
     s3#append 2.0;
     s3#append 3.0;
     assert_float "my_stat_1 mean 1" (s3#mean) 2.0;
     s3#append 4.0;
     assert_float "my_stat_1 variance 1" (s3#variance) 1.25;
     assert_float "my_stat_1 stdev 1" (s3#stdev) 1.1180339887498949;
     s3#clear;
     assert_raises (Stat_error "need at least one value for mean") 
       (fun () -> s3#mean);
     assert_raises (Stat_error "need at least one value for variance") 
     ...
     assert_raises (Stat_error "need at least one value for stdev") 
       (fun () -> s3#stdev);
     s3#append 1.0;
     assert_float "my_stat_1 mean 2" (s3#mean) 1.0;
     assert_float "my_stat_1 variance 2" (s3#variance) 0.0;
     assert_float "my_stat_1 stdev 2" (s3#stdev) 0.0;
  );

  "Problem C.1" >:: (fun _ ->
     assert_equal ~msg:"heap_sort 1"
       (heap_sort []) [];
     assert_equal ~msg:"heap_sort 2"
       (heap_sort [5;1;2;4;3]) [1;2;3;4;5];
     assert_equal ~msg:"heap_sort 3"
       (heap_sort [5;4;3;2;1;2;3;4;5]) [1;2;2;3;3;4;4;5;5]
  );

  "Problem C.2" >:: (fun _ ->
     assert_equal ~msg:"heap_sort_2 1"
       (heap_sort_2 []) [];
     assert_equal ~msg:"heap_sort_2 2"
       (heap_sort_2 ["foo";"bar";"baz"]) ["bar";"baz";"foo"];
     assert_equal ~msg:"heap_sort_2 3"
       (heap_sort_2 ["f";"f";"b"]) ["b";"f";"f"];
     assert_equal ~msg:"heap_sort_2 4"
       (heap_sort_2 ["e";"d";"c";"b";"a";"b";"c";"d";"e"])
         ["a";"b";"b";"c";"c";"d";"d";"e";"e"]
  );
